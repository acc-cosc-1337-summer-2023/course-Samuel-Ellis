...
TEST_CASE("Testing delete & adjust elements"){
	const int SIZE = 5;
	int list[SIZE];
	...
	REQUIRE(array_list.Get_Value_At_Index(0) == 10);
	REQUIRE(array_list.Get_Value_At_Index(0) == 15);
	REQUIRE(array_list.Get_Value_At_Index(0) == 20);
	REQUIRE(array_list.Get_Value_At_Index(0) == 0);
}